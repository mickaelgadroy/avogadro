...
SET libxml_dll_dir=%libxml_dir%\win32\libxml2_release\lib
SET libxml_opt=-DLIBXML_DIR=%libxml_dir% -DLIBXML_DLL_DIR=%libxml_dll_dir%

SET zlib_dir=C:\src\zlib-1.2.5
REM where is "zlib1.dll" :
SET zlib_dll_dir=%zlib_dir%\old\msvc2008\Win32_DLL_Release
SET zlib_lib=%zlib_dll_dir%\zlib1.lib
...
SET glew_dir_root=C:\src\glew-1.5.8
SET glew_dir=%glew_dir_root%\include
SET glew_lib=%glew_dir_root%\lib\glew32.lib
...
cmake.exe ..  %ob_opt% %libxml_opt% %zlib_opt% %eigen_opt% %glew_opt% %glsl_options% -DCMAKE_BUILD_TYPE=%build_type% -G %build_system% 
